...
2. If a local `.agent-os/` control plane exists, read its workspace, project, tasks, decisions, evals, capabilities, dispatch policy, and write policy.
...
6. For reset requests, run `reset-project --dry-run` first. For legacy reorganization, run `migrate-legacy-project --write-plan` first.
