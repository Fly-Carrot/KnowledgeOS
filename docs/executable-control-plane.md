...
- lifecycle consistency for `run-task -> phase-task -> eval-task -> verify-lifecycle -> complete-task`.
...
Close a task only after `eval-task` passed, declared outputs exist, lifecycle verification passes, and required postflight succeeds or records an explicit pending reason.
